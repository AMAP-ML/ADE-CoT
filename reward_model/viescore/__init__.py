import sys
sys.path.insert(0, 'reward_model/viescore')

from utils_viescore import (
    mllm_output_to_dict
)
import math
import vie_prompts
...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None) -> None:
        ...
        if self.backbone_name == "gpt4o":
            ...
            self.model = GPT4o(key_path, model_name="gpt-4o-0806")
        elif self.backbone_name == "gpt4.1":
            from mllm_tools.openai import GPT4o
            self.model = GPT4o(key_path, model_name="gpt-4.1")
        elif self.backbone_name == "gpt4v":
            from mllm_tools.openai import GPT4v
            self.model = GPT4v(key_path, are_images_encoded=True)
        elif self.backbone_name == "qwen25vl":
            from mllm_tools.qwen25vl_api import QwenVL
            self.model = QwenVL(key_path, model_name="qwen2.5-vl-72b-instruct")
        elif self.backbone_name in ["qwen2.5-vl-7b-instruct", "qwen-vl-max", "qwen3-vl-plus", "qwen2.5-vl-72b-instruct"]:
            from mllm_tools.qwen25vl_api import QwenVL
            self.model = QwenVL(key_path, model_name=self.backbone_name)
        else:
            raise NotImplementedError(
                f"backbone `{self.backbone_name}` is not supported. "
                f"Available options: gpt4o, gpt4.1, gpt4v, qwen25vl, qwen-vl-max, "
                f"qwen3-vl-plus, qwen2.5-vl-7b-instruct, qwen2.5-vl-72b-instruct."
            )
    ...
    def evaluate(self, image_prompts, text_prompt, extract_overall_score_only=False, extract_all_score=True, echo_output=False):
        ...
        # print("_SC_prompt: ", _SC_prompt)
        # print("PQ_prompt: ", self.PQ_prompt)
        ...
        while SC_dict is False or PQ_dict is False:
            ...
            # print("\nSC_prompt_final: ", SC_prompt_final)
            # time.sleep(2)
            result_SC = self.model.get_parsed_output(SC_prompt_final, constant_flag=True)
            # print("\nresult_SC: ", result_SC)
            # print(result_SC)

            # time.sleep(2)
            # print("\nPQ_prompt_final: ", PQ_prompt_final)
            result_PQ = self.model.get_parsed_output(PQ_prompt_final, constant_flag=True)
            # print("\nresult_PQ:", result_PQ)
        ...
        if extract_all_score:
            ...
            return [SC_score, PQ_score, O_score, SC_dict, PQ_dict]
...
if __name__ == "__main__":
    model = VIEScore(backbone="gpt4o", task="t2i")
