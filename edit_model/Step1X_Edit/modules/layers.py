...
def attention_after_rope(q, k, v, pe, mode, return_attn_flag=False):
    q, k = apply_rope(q, k, pe)

    if return_attn_flag:
        from .attention import attention_with_cross_attn_score as attention
        x, softmax_lse = attention(q, k, v, mode)
        return x, softmax_lse
    else:
...
def apply_rope(xq, xk, freqs_cis):
    # Restore the (num_heads, seq_len) order expected by the upstream function.
    xq = xq.transpose(1, 2)  # [batch, num_heads, seq_len, head_dim]
    xk = xk.transpose(1, 2)

    # Split head_dim into complex pairs (real, imag).
    xq_ = xq.float().reshape(*xq.shape[:-1], -1, 1, 2)
    xk_ = xk.float().reshape(*xk.shape[:-1], -1, 1, 2)

    # Apply rotary position embedding via complex multiplication.
    xq_out = freqs_cis[..., 0] * xq_[..., 0] + freqs_cis[..., 1] * xq_[..., 1]
    xk_out = freqs_cis[..., 0] * xk_[..., 0] + freqs_cis[..., 1] * xk_[..., 1]

    # Reshape back and transpose to the target layout.
...
class DoubleStreamBlock(nn.Module):
    ...
    def _forward(
        ...
        if return_attn_flag:
            attn, softmax_lse = attention_after_rope(q, k, v, pe, self.mode, return_attn_flag)
            txt_softmax_lse = softmax_lse[:, :, :txt.shape[1]]
            img_softmax_lse = softmax_lse[:, :, txt.shape[1]:]
        else:
        ...
        if return_attn_flag:
            return img, txt, img_softmax_lse, txt_softmax_lse
        else:
            return img, txt
