...
def teacache_transformer(pipe):
    ...
    def new_forward(
        ...
        if self.cnt == 0 or self.cnt == self.num_steps-1:
            ...
            # Check whether tensor shapes match.
            if (self.previous_modulated_input is not None and 
                modulated_inp.shape == self.previous_modulated_input.shape):
                ...
                # Shapes do not match, force recomputation.
                should_calc = True
                self.accumulated_rel_l1_distance = 0
        ...
        if self.cnt == 0 or self.cnt == self.num_steps - 1:
            ...
            if (self.previous_modulated_input is not None and 
                ...
                if self.previous_modulated_input is not None:
                    print(f"[parallel_teacache] shape mismatch: modulated_inp.shape={modulated_inp.shape}, previous_modulated_input.shape={self.previous_modulated_input.shape}")
                else:
                    print(f"[parallel_teacache] previous_modulated_input is None, forcing recomputation")
