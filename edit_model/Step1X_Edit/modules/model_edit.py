...
# ==================== Constant Parameters ==================== #
# ----- Visualization flags ----- #
visualize_cross_attn_flag = False 

# ----- Head pooling strategy ----- #
attn_pool_way = "max"
...
class Step1XEdit(nn.Module):
    ...
    def forward(
        ...
        # ----- Cross-attention visualization flag ----- #
        if visualize_cross_attn_flag:

            img_softmax_lse_all = None
            txt_softmax_lse_all = None 

        if not self.blocks_to_swap:

            for idx_block, block in enumerate(self.double_blocks):
                
                if visualize_cross_attn_flag:
                    img, txt, img_softmax_lse, txt_softmax_lse = block(img=img, txt=txt, vec=vec, pe=pe)

                    if idx_block > 8 and idx_block < 11:
                        if attn_pool_way == "max":
                            img_softmax_lse, _ = img_softmax_lse.max(dim=1)
                            txt_softmax_lse, _ = txt_softmax_lse.max(dim=1)
                        elif attn_pool_way == "mean":
                            img_softmax_lse = img_softmax_lse.mean(dim=1)
                            txt_softmax_lse = txt_softmax_lse.mean(dim=1)
                        else:
                            print(f"Unknown attn_pool_way: {attn_pool_way}! Error!")
                            sys.exit()

                        if img_softmax_lse_all is None:
                            img_softmax_lse_all = img_softmax_lse.unsqueeze(1)
                            txt_softmax_lse_all = txt_softmax_lse.unsqueeze(1)
                        else:
                            img_softmax_lse_all = torch.cat((img_softmax_lse_all, img_softmax_lse.unsqueeze(1)), dim=1)
                            txt_softmax_lse_all = torch.cat((txt_softmax_lse_all, txt_softmax_lse.unsqueeze(1)), dim=1)
            ...
            print("\nThis code path has not been adapted yet!")
            sys.exit()
        ...
        if visualize_cross_attn_flag:
            visualize_att_map_dict = {} 

            visualize_att_map_dict["img_softmax_lse_all"] = img_softmax_lse_all.mean(dim=1)
            visualize_att_map_dict["txt_softmax_lse_all"] = txt_softmax_lse_all.mean(dim=1)

            # print("img_softmax_lse_all: ", visualize_att_map_dict["img_softmax_lse_all"].shape)
            # print("txt_softmax_lse_all: ", visualize_att_map_dict["txt_softmax_lse_all"].shape)


            return img, visualize_att_map_dict
        else:
            return img
