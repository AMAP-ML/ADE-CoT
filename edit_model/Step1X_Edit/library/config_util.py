...
@dataclass
class EditSubsetParams(BaseSubsetParams):
    metadata_file: Optional[str] = None
    alpha_mask: bool = False
...
@dataclass
class EditDatasetParams(BaseDatasetParams):
    batch_size: int = 1
    enable_bucket: bool = False
    min_bucket_reso: int = 256
    max_bucket_reso: int = 1024
...
class SubsetBlueprint:
    params: Union[DreamBoothSubsetParams, FineTuningSubsetParams, EditSubsetParams]
...
class DatasetBlueprint:
    ...
    is_controlnet: bool
    is_edit: bool
    params: Union[DreamBoothDatasetParams, FineTuningDatasetParams, EditSubsetParams]
...
class ConfigSanitizer:
    ...
    def __init__(self, support_dreambooth: bool, support_finetuning: bool, support_controlnet: bool, support_dropout: bool, support_edit: bool) -> None:
    ...
    def sanitize_user_config(self, user_config: dict) -> dict:
        try:
            ...
            # TODO: surface a clearer message when an error occurs.
...
class BlueprintGenerator:
    ...
    def generate(self, user_config: dict, argparse_namespace: argparse.Namespace, **runtime_params) -> Blueprint:
        ...
        for dataset_config in sanitized_user_config.get("datasets", []):
            ...
            is_edit = dataset_config.get('edit_dataset', False)
            if is_controlnet:
                ...
                subset_params_klass = EditSubsetParams
                dataset_params_klass = EditDatasetParams
            else:
            ...
            dataset_blueprints.append(DatasetBlueprint(is_dreambooth, is_controlnet, is_edit, params, subset_blueprints))
...
def generate_dataset_group_by_blueprint(dataset_group_blueprint: DatasetGroupBlueprint) -> Tuple[DatasetGroup, Optional[DatasetGroup]]:
    ...
    for dataset_blueprint in dataset_group_blueprint.datasets:
        ...
        elif dataset_blueprint.is_edit:
            subset_klass = EditSubset
            dataset_klass = EditDataset
        else:
    ...
    val_datasets: List[Union[DreamBoothDataset, FineTuningDataset, EditDataset, ControlNetDataset]] = []
    for dataset_blueprint in dataset_group_blueprint.datasets:
    ...
    def print_info(_datasets, dataset_type: str):
        ...
        for i, dataset in enumerate(_datasets):
            ...
            is_edit = isinstance(dataset, EditDataset)
            info += dedent(f"""\
                [{dataset_type} {i}]
...
def load_user_config(file: str) -> dict:
    ...
    if file.name.lower().endswith(".json"):
        try:
            ...
            logger.error(
                f"Error on parsing JSON config file. Please check the format: {file}"
            )
            raise
        ...
        try:
            ...
            logger.error(
                f"Error on parsing TOML config file. Please check the format: {file}"
            )
            raise
    else:
