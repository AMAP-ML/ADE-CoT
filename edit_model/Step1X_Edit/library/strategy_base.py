    ...
    def _get_input_ids(
        ...
        if max_length > tokenizer.model_max_length:
            ...
            if tokenizer.pad_token_id == tokenizer.eos_token_id:
                # v1
                # When length is > 77 the tokens look like "<BOS> .... <EOS> <EOS> <EOS>" totalling e.g. 227,
                # so we split them into chunks of "<BOS>...<EOS>".
                # AUTOMATIC1111's implementation splits at commas etc., but here we do a simple fixed-size split.
                ...
                # v2 or SDXL
                # When length is > 77 the tokens look like "<BOS> .... <EOS> <PAD> <PAD>..." totalling e.g. 227,
                # so we split them into chunks of "<BOS>...<EOS> <PAD> <PAD> ...".
                for i in range(1, max_length - tokenizer.model_max_length + 2, tokenizer.model_max_length - 2):
                    ...
                    # If the tail is x <PAD/EOS>, replace the last token with <EOS> (no change when x <EOS>).
                    if ids_chunk[-2] != tokenizer.eos_token_id and ids_chunk[-2] != tokenizer.pad_token_id:
                        ids_chunk[-1] = tokenizer.eos_token_id
                    # If the head is <BOS> <PAD> ..., replace with <BOS> <EOS> <PAD> ...
