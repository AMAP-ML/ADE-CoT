...
class NetworkTrainer:
    ...
    # TODO consolidate with the other scripts.
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        raise NotImplementedError("load_target_model is not implemented for this model")

    def get_tokenize_strategy(self, args):
        raise NotImplementedError("get_tokenize_strategy is not implemented for this model")

    def get_tokenizers(self, tokenize_strategy) -> List[Any]:
        return [tokenize_strategy.tokenizer]

    def get_latents_caching_strategy(self, args):
        raise NotImplementedError("get_latents_caching_strategy is not implemented for this model")

    def get_text_encoding_strategy(self, args):
        raise NotImplementedError("get_text_encoding_strategy is not implemented for this model")
    ...
    def get_sai_model_spec(self, args):
        raise NotImplementedError("get_sai_model_spec is not implemented for this model")
    ...
    def process_batch(
        ...
        with torch.no_grad():
            if "latents" in batch and batch["latents"] is not None:
                ...
                ref_latents = typing.cast(torch.FloatTensor, batch["ref_latents"].to(accelerator.device))
            else:
                # Convert to latent.
                if args.vae_batch_size is None or len(batch["images"]) <= args.vae_batch_size:
                    latents = self.encode_images_to_latents(args, vae, batch["images"].to(accelerator.device, dtype=vae_dtype))
                    ref_latents = self.encode_images_to_latents(args, vae, batch['ref_images'].to(accelerator.device,dtype=vae_dtype))
                else:
                    chunks = [
                        batch["images"][i : i + args.vae_batch_size] for i in range(0, len(batch["images"]), args.vae_batch_size)
                    ]
                    ref_chunks = [
                        batch["ref_images"][i : i + args.vae_batch_size] for i in range(0, len(batch["ref_images"]), args.vae_batch_size)
                    ]
                    list_latents = []
                    ref_list_latents = []
                    for chunk, ref_chunk in zip(chunks,ref_chunks):
                        with torch.no_grad():
                            ...
                            ref_chunk = self.encode_images_to_latents(args, vae, ref_chunk.to(accelerator.device, dtype=vae_dtype))
                            ref_list_latents.append(ref_chunk)
                    latents = torch.cat(list_latents, dim=0)
                    ref_latents = torch.cat(ref_list_latents, dim=0)

                # Warn and replace with zero if NaNs are present.
                if torch.any(torch.isnan(latents)):
                    accelerator.print("NaN found in latents, replacing with zeros")
                    latents = typing.cast(torch.FloatTensor, torch.nan_to_num(latents, 0, out=latents))

            latents = self.shift_scale_latents(args, latents)
            ref_latents = self.shift_scale_latents(args, ref_latents)
    ...
    def train(self, args):
        ...
        if args.dataset_class is None:
            blueprint_generator = BlueprintGenerator(ConfigSanitizer(True, True, args.masked_loss, True, True))
            if use_user_config:
        ...
        # Prepare the dtype matching mixed precision and cast as needed.
        ...
        # Load the model for delta-additive learning.
        sys.path.append(os.path.dirname(__file__))
        accelerator.print("import network module:", args.network_module)
        network_module = importlib.import_module(args.network_module)

        if args.base_weights is not None:
            # If base_weights is specified, load the given weights and merge them in.
        ...
        # Cache text-encoder outputs if needed; the text encoder is moved to CPU or GPU.
        ...
        # Prepare the classes required for training.
        ...
        #     accelerator.print("no trainable parameters found")
        ...
        # Number of DataLoader workers: note that 0 disables persistent_workers.
        ...
        # Also notify the dataset of the training step count.
        train_dataset_group.set_max_train_steps(args.max_train_steps)

        # Set up the learning-rate scheduler.
        lr_scheduler = train_util.get_scheduler_fix(args, optimizer, accelerator.num_processes)

        # Experimental: train in fp16/bf16 including gradients, casting the whole model to fp16/bf16.
        if args.full_fp16:
            assert (
        ...
        if not cache_latents:  # If caching is disabled, prepare the VAE because we will use it.
            vae.requires_grad_(False)
            vae.eval()
            vae.to(accelerator.device, dtype=vae_dtype)

        # Experimental: train in fp16 including gradients; patch PyTorch to enable grad scaling under fp16.
        if args.full_fp16:
        ...
        accelerator.print(f"  num train images * repeats")
        accelerator.print(
            f"  num validation images * repeats"
        )
        accelerator.print(f"  num reg images")
        accelerator.print(f"  num batches per epoch")
        ...
        accelerator.print(f"  gradient accumulation steps")
        accelerator.print(f"  total optimization steps")
        ...
        if use_user_config:
            ...
            for dataset in train_dataset_group.datasets:
                ...
                for ds_dir_name, ds_freq_for_dir in dataset.tag_frequency.items():
                    # When a directory is used by multiple datasets, count it only once.
                    # The number of times the tag appears in the caption and how often it is used in training may not match,
                    # so summing counts across multiple datasets does not provide much meaning.
