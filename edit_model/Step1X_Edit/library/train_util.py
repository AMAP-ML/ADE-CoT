...
class ImageInfo:
    def __init__(self, image_key: str, num_repeats: int, caption: str, is_reg: bool, absolute_path: str) -> None:
        # for image edit only
        self.ref_absolute_path: Optional[str] = None  # absolute path of the reference image
        self.ref_alpha_mask: Optional[torch.Tensor] = None  # alpha mask can be flipped in runtime
        self.ref_latents: Optional[torch.Tensor] = None
        self.ref_latents_flipped: Optional[torch.Tensor] = None
        self.ref_image: Optional[Image.Image] = None  # optional, original PIL Image
        # kohya
...
class BucketManager:
    def __init__(self, no_upscale, max_reso, min_size, max_size, reso_steps) -> None:
        ...
        self.resos = []
        self.reso_to_id = {}
        self.buckets = []  # at preprocess: (image_key, image, original size, crop left/top); at train time: image_key
    ...
    def sort(self):
        # Sort by resolution (purely for nicer display and metadata). Reorder buckets and rebuild reso_to_id accordingly.
    ...
    def set_predefined_resos(self, resos):
        # Store resolutions and aspect ratios for the case where we pick from predefined sizes.
    ...
    def select_bucket(self, image_width, image_height):
        ...
        if not self.no_upscale:
            # Both upscaling and downscaling are allowed.
            # Multiple buckets may share the same aspect ratio (e.g. fine tuning with no_upscale=True), so prefer the one with the same resolution.
            reso = (image_width, image_height)
            if reso in self.predefined_resos_set:
                pass
            else:
                ar_errors = self.predefined_aspect_ratios - aspect_ratio
                predefined_bucket_id = np.abs(ar_errors).argmin()  # the one with the smallest aspect-ratio error (excluding the same resolution)
                reso = self.predefined_resos[predefined_bucket_id]

            ar_reso = reso[0] / reso[1]
            if aspect_ratio > ar_reso:  # wider than the bucket -> match the height
            ...
            if image_width * image_height > self.max_area:
                # The image is too large, so choose a bucket assuming we will downscale while preserving aspect ratio.
                resized_width = math.sqrt(self.max_area * aspect_ratio)
                resized_height = self.max_area / resized_width
                assert abs(resized_width / resized_height - aspect_ratio) < 1e-2, "aspect is illegal"

                # After resizing, round the short or long side to reso_steps: pick whichever yields the smaller aspect-ratio diff.
            ...
            # Use a bucket size smaller than the image (we crop instead of padding).
    ...
    def get_crop_ltrb(bucket_reso: Tuple[int, int], image_size: Tuple[int, int]):
        # Compute crop left/top to match Stability AI's preprocessing. crop right is computed to support flip augmentation.
        # Calculate crop left/top according to the preprocessing of Stability AI. Crop right is calculated for flip augmentation.

        bucket_ar = bucket_reso[0] / bucket_reso[1]
        image_ar = image_size[0] / image_size[1]
        if bucket_ar > image_ar:
            # The bucket is wider -> match the height.
...
class AugHelper:
    # Removed the dependency on albumentations, but keep the same interface for now.
...
class BaseSubset:
    def __init__(
        ...
        self.token_warmup_step = token_warmup_step  # the number of tags reaches its maximum at step N (or N*max_train_steps if N<1)
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.current_epoch: int = 0  # instances are recreated per epoch, so this must be passed in from outside
        ...
        # caching
        self.caching_mode = None  # None, 'latents', 'text'; commented by hyc, this does not work
    ...
    def set_current_epoch(self, epoch):
        if not self.current_epoch == epoch:  # shuffle buckets when the epoch changes
    ...
    def process_caption(self, subset: BaseSubset, caption):
        # Append prefix/suffix to the caption.
        if subset.caption_prefix:
            caption = subset.caption_prefix + " " + caption
        if subset.caption_suffix:
            caption = caption + " " + subset.caption_suffix

        # Decide dropout here: tag drop lives in this method, so this is the right place.
        ...
        if is_drop_out:
            ...
            if subset.shuffle_caption or subset.token_warmup_step > 0 or subset.caption_tag_dropout_rate > 0:
                ...
                if subset.token_warmup_step < 1:  # overwrite on first iteration
    ...
    def get_input_ids(self, caption, tokenizer=None):
        ...
        if self.tokenizer_max_length > tokenizer.model_max_length:
            ...
            if tokenizer.pad_token_id == tokenizer.eos_token_id:
                # v1
                # When length is > 77 the tokens look like "<BOS> .... <EOS> <EOS> <EOS>" totalling e.g. 227, so split into chunks of "<BOS>...<EOS>".
                # AUTOMATIC1111's implementation splits at commas etc.; here we simply split into fixed-size chunks.
                for i in range(
                ...
                # v2 or SDXL
                # When length is > 77 the tokens look like "<BOS> .... <EOS> <PAD> <PAD>..." totalling e.g. 227, so split into chunks of "<BOS>...<EOS> <PAD> <PAD> ...".
                for i in range(1, self.tokenizer_max_length - tokenizer.model_max_length + 2, tokenizer.model_max_length - 2):
                    ...
                    # If the tail is <EOS> <PAD> or <PAD> <PAD>, do nothing.
                    # If the tail is x <PAD/EOS>, set the last token to <EOS> (no change when x <EOS>).
                    if ids_chunk[-2] != tokenizer.eos_token_id and ids_chunk[-2] != tokenizer.pad_token_id:
                        ids_chunk[-1] = tokenizer.eos_token_id
                    # If the head is <BOS> <PAD> ..., change it to <BOS> <EOS> <PAD> ...
    ...
    def make_buckets(self):
        """
        Must be called even when bucketing is disabled (a single bucket is created).
        ...
        # Create buckets and assign images to them.
        if self.enable_bucket:
            if self.bucket_manager is None:  # already initialized for fine tuning when metadata provides bucket definitions
        ...
        # Build the index for accessing data; this index is used for dataset shuffling.
    ...
    def cache_latents(self, vae, vae_batch_size=1, cache_to_disk=False, is_main_process=True, file_suffix=".npz"):
        # Multi-GPU is not supported here; use tools/cache_latents.py for that.
        logger.info("caching latents.")
    ...
    def crop_target(self, subset: BaseSubset, image, face_cx, face_cy, face_w, face_h):
        ...
        # Image is larger than the target size, so resize it.
        face_size = max(face_w, face_h)
        size = min(self.height, self.width)  # shorter side
        min_scale = max(self.height / height, self.width / width)  # smallest scale that makes the image match the model input size
        min_scale = min(1.0, max(min_scale, size / (face_size * subset.face_crop_aug_range[1])))  # min face size specified
        max_scale = min(1.0, max(min_scale, size / (face_size * subset.face_crop_aug_range[0])))  # max face size specified
        if min_scale >= max_scale:  # range specifies min == max
            scale = min_scale
        else:
        ...
        # Crop to e.g. 448x640 centered around the face.
        for axis, (target_size, length, face_p) in enumerate(zip((self.height, self.width), (height, width), (face_cy, face_cx))):
            p1 = face_p - target_size // 2  # crop offset to center the face

            if subset.random_crop:
                # Slightly shift the crop so the background is also included while keeping the face roughly centered.
                range = max(length - face_p, face_p)  # the larger distance from image edge to face center
                p1 = p1 + (random.randint(0, range) + random.randint(0, range)) - range  # a smooth random value in [-range, +range]
            else:
                # Add a little randomness only when a range is specified (intentionally rough).
    ...
    def __getitem__(self, index):
        ...
        crop_top_lefts = []
        target_sizes_hw = []
        flippeds = []  # the variable name is a bit awkward
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:  # when cache_latents=True
                ...
                image = None
            elif image_info.latents_npz is not None:  # for FineTuningDataset or when cache_latents_to_disk=True
                ...
                # )
                latents, ref_latents, original_size, crop_ltrb, flipped_latents, ref_flipped_latents, alpha_mask, ref_alpha_mask = self.latents_caching_strategy.load_latents_from_disk(
                    image_info.latents_npz, image_info.bucket_reso
                )

                latents = torch.FloatTensor(latents)
                ref_latents = torch.FloatTensor(ref_latents)
                if alpha_mask is not None:
                    alpha_mask = torch.FloatTensor(alpha_mask)
                    ref_alpha_mask = torch.FloatTensor(ref_alpha_mask)
                ...
                if self.enable_bucket:
                    ...
                    if face_cx > 0:  # face position info is available
                ...
                latents = None
                image = self.image_transforms(img)  # produces a torch.Tensor in [-1.0, 1.0]
                del img

            images.append(image)
            ref_images.append(ref_image)
            latents_list.append(latents)
            ref_latents_list.append(ref_latents)
            alpha_mask_list.append(alpha_mask)
            ref_alpha_mask_list.append(ref_alpha_mask)
        ...
        example["ref_images"] = ref_images

        example["latents"] = torch.stack(latents_list) if latents_list[0] is not None else None
        example["ref_latents"] = torch.stack(ref_latents_list) if ref_latents_list[0] is not None else None
...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        if self.enable_bucket:
            ...
            self.min_bucket_reso = None
            self.max_bucket_reso = None
            self.bucket_reso_steps = None  # this info is unused
            self.bucket_no_upscale = False

        def read_caption(img_path, caption_extension, enable_wildcard):
            # Build candidate file names for the caption.
                ...
                if os.path.isfile(cap_path):
                    with open(cap_path, "rt", encoding="utf-8") as f:
                        try:
                            ...
                            logger.error(f"illegal char in file (not UTF-8)")
                            raise e
                        assert len(lines) > 0, f"caption file is empty"
                        if enable_wildcard:
                            caption = "\n".join([line.strip() for line in lines if line.strip() != ""])  # drop blank lines and join with newlines
                        else:
        ...
        def load_dreambooth_dir(subset: DreamBoothSubset):
            ...
            if use_cached_info_for_subset:
                ...
                # Load a prompt per image file and use it when available.
                captions = []
                missing_captions = []
            ...
            self.set_tag_frequency(os.path.basename(subset.image_dir), captions)  # record tag frequencies
            ...
            if not use_cached_info_for_subset and subset.cache_info:
                logger.info(f"cache image info for")
                ...
                logger.info(f"cache image info done for")
        ...
        if num_reg_images == 0:
            ...
            # Compute num_repeats: small enough that a simple loop is fine.
            n = 0
            first_loop = True
...
class FineTuningDataset(BaseDataset):
    def __init__(
        ...
        for subset in subsets:
            ...
            for image_key, img_md in metadata.items():
                ...
                if os.path.exists(image_key):
                    ...
                    # A bit hacky, but no better idea comes to mind.
                    paths = glob_images(subset.image_dir, image_key)
                    if len(paths) > 0:
                        abs_path = paths[0]

                # Fall back to npz files if no image is found.
                if abs_path is None:
                ...
                assert abs_path is not None, f"no image"
        ...
        if sizes is None:
            ...
            # Initialize bucket info now so make_buckets does not recreate it.
            self.bucket_manager = BucketManager(False, None, None, None, None)
            self.bucket_manager.set_predefined_resos(resos)

        # Clean up the npz info.
...
class EditDataset(BaseDataset):
    def __init__(
        self,
        subsets: Sequence[EditSubset],
        batch_size: int,
        resolution,
        network_multiplier: float,
        enable_bucket: bool,
        ...
        for subset in subsets:
            ...
            for image_key, img_md in metadata.items():
                ...
                ref_image_path = img_md.get("ref_image_path")
                if not os.path.exists(ref_image_path):
                    raise ValueError(f"{ref_image_path} does not exists")
                ...
                image_info = ImageInfo(image_key, subset.num_repeats, caption, False, abs_path)
                image_info.image_size = img_md.get("train_resolution")
                image_info.ref_absolute_path = ref_image_path
    ...
    def image_key_to_npz_file(self, subset: EditSubset, image_key):
        base_name = os.path.splitext(image_key)[0]
        npz_file_norm = base_name + ".npz"
...
class ControlNetDataset(BaseDataset):
    def __init__(
        ...
        # Store values referenced by config_util etc. (a bit awkward, would like to clean this up).
        ...
        assert (
            len(missing_imgs) == 0
        ), f"missing conditioning data for {len(missing_imgs)} images"
        assert (
            len(extra_imgs) == 0
        ), f"extra conditioning data for {len(extra_imgs)} images"
    ...
    def __getitem__(self, index):
        ...
        for i, image_key in enumerate(bucket[image_index : image_index + bucket_batch_size]):
            ...
            if self.dreambooth_dataset_delegate.enable_bucket:
                ...
                ), f"size of conditioning image is not match"
                ...
                # Currently only center crop is supported (not random crop).
                h, w = target_size_hw
...
def is_disk_cached_latents_is_expected(reso, npz_path: str, flip_aug: bool, alpha_mask: bool):
    expected_latents_size = (reso[1] // 8, reso[0] // 8)  # note: bucket_reso is W x H
...
def debug_dataset(train_dataset, show_input_ids=False):
    logger.info(f"Total dataset length (steps)")
    logger.info(
...
# Load an image. Returns numpy.ndarray, (original width, original height), (crop left, crop top, crop right, crop bottom).
def trim_and_resize_if_required(
    ...
    # No clear idea on how to reflect random-crop offsets into crop left/top.
...
def load_images_and_masks_for_caching(
    ...
    for info in image_infos:
        ...
        # TODO add a check: image metadata may be corrupted so the bucket assigned from metadata might not match the actual image size.
...
# region module-replacement section
"""
Module replacement to speed things up.
"""

# CrossAttention that uses FlashAttention.
...
def replace_vae_modules(vae: diffusers.models.AutoencoderKL, mem_eff_attn, xformers):
    ...
    if mem_eff_attn:
        ...
        # Use Diffusers' xformers for now. Attention exists only in the MidBlock.
...
def add_dit_training_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        ""
        "Increasing this value reduces VRAM usage during training but also slows down training (s/it).",
    )
...
def verify_command_line_training_args(args: argparse.Namespace):
    ...
    # When wandb is enabled, the command line is exposed.
    # Check whether the training command-line arguments include any sensitive options,
    # and warn or notify the user to move them into a config file when they are.
...
def verify_training_args(args: argparse.Namespace):
    ...
    Validate training arguments and also reflect the highvram option into a global variable.
    """
    ...
    # # Could count using a List but let's just lay them out.
...
def add_dataset_arguments(
    ...
    parser.add_argument(
        ...
        help="Resize interpolation when required. Default: area Options: lanczos, nearest, bilinear, bicubic, area",
    )
    parser.add_argument(
        "--token_warmup_min",
        type=int,
    ...
    if support_caption_dropout:
        ...
        # To avoid confusion with tensor Dropout, prefix with 'caption'. every_n_epochs defaults to None to match other args.
        parser.add_argument(
...
def get_optimizer(args, trainable_params) -> tuple[str, str, object]:
    ...
    if optimizer_type == "Lion".lower():
        ...
        # Validate args and adjust them as needed.
        ...
        if optimizer_kwargs["relative_step"]:
            ...
            # Handle the case where trainable_params is a group: remove lr.
            if type(trainable_params) == list and type(trainable_params[0]) == dict:
                has_group_lr = False
                for group in trainable_params:
                    p = group.pop("lr", None)
                ...
                if has_group_lr:
                    # Disable args for safety. TODO this inverts the dependency, which is not ideal.
            ...
            args.lr_scheduler = f"adafactor:{lr}"  # a bit awkward
    ...
    if optimizer is None:
        # Use a user-specified optimizer.
    ...
    if args.optimizer_schedulefree_wrapper and not optimizer_type.endswith("schedulefree".lower()):
        ...
        class OptimizerProxy(torch.optim.Optimizer):
            ...
            # Method to pass isinstance checks.
...
def prepare_accelerator(args: argparse.Namespace):
    ...
    # torch.compile option. NO means do not use torch.compile.
    dynamo_backend = "NO"
...
def get_hidden_states(args: argparse.Namespace, input_ids, tokenizer, text_encoder, weight_dtype=None):
    ...
    if args.max_token_length is not None:
        if args.v2:
            # v2: restore <BOS>...<EOS> <PAD> ... chunks back into <BOS>...<EOS> <PAD> ...; not sure if this implementation is right.
            states_list = [encoder_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            for i in range(1, args.max_token_length, tokenizer.model_max_length):
                chunk = encoder_hidden_states[:, i : i + tokenizer.model_max_length - 2]  # from after <BOS> to before the end
                if i > 0:
                    for j in range(len(chunk)):
                        if input_ids[j, 1] == tokenizer.eos_token:  # empty, i.e. the <BOS> <EOS> <PAD> ... pattern
                            chunk[j, 0] = chunk[j, 1]  # copy the next <PAD> value
            ...
            # v1: restore <BOS>...<EOS> chunks back into <BOS>...<EOS>.
            states_list = [encoder_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            for i in range(1, args.max_token_length, tokenizer.model_max_length):
                states_list.append(
...
def get_remove_step_no(args: argparse.Namespace, step_no: int):
    ...
    # From step_no = current - last_n_steps, compute step_no values that are multiples of save_every_n_steps and delete them.
    # e.g. with save_every_n_steps=10 and save_last_n_steps=30, at step 50 keep the last 30 steps and remove step 10.
...
def get_my_scheduler(
    *,
    ...
    elif sample_sampler == "ddpm":  # ddpm breaks, so it is excluded from the options
...
# Used by collate_fn; epoch and step are multiprocessing.Value
